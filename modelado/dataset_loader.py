...
class DataBuilder:
    ...
    def _load_and_align_data(self):
        ...
        available_vars = {
            "TO": temp, "TOB": temp_bottom, "CHL": chl, "MLOTST": mixed,
            "ZO": zo, "SO": so, "UGO": ugo, "VGO": vgo, "PP": pp,
            "CDM": cdm, "SPM": spm, "ZSD": zsd, "PROF": depth,
            "MSEN": month_sin, "MCOS": month_cos, "month": month, 
            "Año": year, "LAT": lat, "LON": lon, "FL": moon_phase
        }
        ...
        # Align only the selected variables (Faster and saves memory)
    ...
    def _create_windows(self):
        ...
        # forecast_lead=1 -> offset 0 -> current behavior
        # forecast_lead=2 -> offset 1 -> skip one month
        lead_offset = self.forecast_lead - 1

        n_windows = (len(X_data)-self.window_size-lead_offset-self.prediction_horizon)

        for i in range(n_windows+1):

            input_start = i
            input_end = i + self.window_size

            target_start = input_end + lead_offset
            ...
            time_seq.append(time_data[target_start:target_end])
        ...
        return X_seq, y_seq, m_seq, time_seq
    ...
    def _create_test_windows(self):
        ...
        # Earliest target_start needed so that horizon H can predict first test month
        first_target_start = test_start_idx - (self.prediction_horizon - 1)

        # Latest target_start needed so that horizon +1 can predict last test month
        ...
        for target_start in range(first_target_start, last_target_start + 1):
            ...
            for h in range(self.prediction_horizon):
                ...
                # Only evaluate targets that actually belong to TEST
    ...
    def get_splits(self):
        ...
        # TRAIN: all targets must be before split_year
        train_mask = np.all(years < self.split_year, axis=1)

        # VALIDATION: all targets must be inside validation period
        ...
        # TEST: special rolling-origin evaluation
        X_test, y_test, m_test, t_test = self._create_test_windows()
